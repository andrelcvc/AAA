 ...
 * @param <K> tipo da chave primária da entidade de negócio
 */
public abstract class DaoSupport<E extends PersistentEntity<K>, K extends Serializable> {

    @PersistenceContext(type = PersistenceContextType.EXTENDED)
    private EntityManager entityManager;
    private Class<E> entityClass = null;
    
    private static final Logger logger = Logger.getLogger(DaoSupport.class.getName());
     ...
     * @return entidade
     */
    public E findByPrimaryKey(K pk) {
        logger.log(Level.INFO,"============= Chamando findByPrimarykey ==========");
        if (pk == null) {
            throw new DAOException("A chave primaria não pode ser nula");
        }
        try {
        	logger.log(Level.INFO,"EntityManager = " + entityManager);
        	logger.log(Level.INFO,"Entity class = " + getEntityClass());
